...
<head>
    ...
    <title>Visualizador - ContainerCheck</title>
    ...
    <nav class="navbar navbar-expand-lg">
        <div class="container-fluid">
            ...
            <div class="d-flex align-items-center">
                <span class="badge bg-secondary text-white me-3">Perfil: Visualizador</span>
                <form action="{{ route('logout') }}" method="POST" class="d-inline">
                    @csrf
                    <button type="submit" class="btn btn-sm btn-outline-danger">Cerrar Sesión</button>
                </form>
            </div>
            <a href="{{ route('visualizador.inventario') }}" class="btn btn-sm btn-outline-primary">Inventario</a>
        </div>
    </nav>
                    ...
                    <tbody>
                        @forelse($responses as $response)
                            <tr>
                                ...
                                <td>
                                    <a href="{{ route('visualizador.show', $response->id) }}"
