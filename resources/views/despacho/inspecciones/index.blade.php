        ...
        <div class="container-fluid">
           <a class="navbar-brand" href="{{ route('despacho.index') }}">
            ...
            <div class="d-flex align-items-center">
                ...
                <a href="{{ route('despacho.index') }}" class="btn btn-sm btn-outline-secondary">Salir</a>
            </div>
        </div>
    </nav>
    ...
    <div class="container mt-4" style="max-width: 700px;">
        ...
        <div id="phase-final" class="phase-card">
            ...
            @else
                ...
                <form action="{{ route('despacho.store', $response->id) }}" method="POST" id="finalForm">
                    @csrf
    ...
    <script>
        ...
        async function saveCurrentPhase(e, form) {
            ...
            if (form.dataset.hasRejection === "true") {
                ...
                try {
                    const res = await fetch("{{ route('despacho.inspecciones.savePhase', $response->id) }}", {
                        method: 'POST', body: formData,
                        headers: { 'X-Requested-With': 'XMLHttpRequest', 'X-CSRF-TOKEN': document.querySelector('input[name="_token"]').value }
                    });
                    if (res.ok) window.location.href = "{{ route('despacho.index') }}";
